...
class SelectAction(BaseModel):
    type: Literal["select"]
    dropdown: str = Field(min_length=1, description="Dropdown combobox name")
    option: str = Field(min_length=1, description="Option text to select")
...
class TextPlanner:
    """LLM-based planner for web automation using only axtree"""
...
You have access to a screenshot of the current page.

Use the screenshot to:
1. **Verify Element Prominence**: Identify which elements are visually dominant
2. **Understand Layout**: See spatial relationships and page structure
3. **Resolve Ambiguity**: When multiple elements have similar names, use visual cues
4. **Detect Visibility**: Confirm elements are actually visible and interactive

Visual Cues to Consider:
- Size and position (larger, centered elements are usually primary)
- Color and styling (bright colors, contrasts indicate importance)
- Grouping (related elements are visually grouped)
- Icons and imagery (help identify element purpose)

Combine Visual + Structural Information:
- Screenshot shows WHAT users see
- Accessibility tree shows WHAT is interactive
- PLEASE Use BOTH to make the best decision

Example Decision Process:
1. Look at screenshot: "I see a large search box at the top center"
2. Check accessibility tree: "[searchbox] name: 'Search Amazon'"
3. Conclusion: "This is clearly the primary search - large, centered, and labeled"

When Elements Are Similar:
- Screenshot: Shows two search boxes (one large, one small)
- Tree: [searchbox] "Search Amazon", [searchbox] "Search in results"
- Decision: Use the larger, more prominent one for initial search

Remember: Trust your visual analysis when it provides clarity, but always verify the element exists in the accessibility tree.
"""
    ...
    def decide(
        ...
        """
        Decide what action to take using vision and text information
        ...
        if not screenshot_base64:
            print("⚠️  Warning: VisionPlanner called without screenshot, falling back to text-only")
            return self._decide_text_only(user_goal, page_state, action_history)
...
=== YOUR TASK ===
Analyze the screenshot and page state to decide the next action.

Use the visual information to:
- Identify the most prominent/relevant elements
- Understand page layout and hierarchy
- Resolve ambiguity when multiple similar elements exist

Provide:
1. An overall plan to achieve the goal
2. Your current step number
3. Detailed reasoning (including visual observations)
4. The specific action to execute"""
        ...
        try:
            ...
            print(f"❌ Vision planning failed: {e}")
            import traceback
            traceback.print_exc()
            
            # Fallback to text-only
            print("⚠️  Falling back to text-only planning...")
            return self._decide_text_only(user_goal, page_state, action_history)
    
    def _decide_text_only(
        self,
        user_goal: str,
        page_state: Dict[str, Any],
        action_history: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Fallback to text-only decision if screenshot unavailable"""
        history_text = self._format_history(action_history)
        state_text = self._format_page_state(page_state)
        self.system_prompt=self._load_system_prompt()  # Reload system prompt without vision instructions
...
{elements_text}{footer}"""
    ...
    def _print_decision(self, result: Dict[str, Any]):
        ...
        if action['type'] == 'goto':
            ...
            print(f" '{action['option']}' from '{action['dropdown']}'")
        elif action['type'] == 'done':
            print(" ✅")
        else:
            print()